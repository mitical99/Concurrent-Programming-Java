...
public class Matches extends Thread {
	...
	public Matches(Semaphore table, Semaphore waitForProducts, Semaphore wakeUpAgent, AtomicBoolean[] shared) {
		super();
		this.table = table;
	...
	public void run() {
		while(true) {
			...
			if(shared[0].get() && shared[1].get()) {
				shared[0].set(false);
				shared[1].set(false);
				System.out.println("Matches smoker took ");
				table.release();
				wakeUpAgent.release();
				try {
